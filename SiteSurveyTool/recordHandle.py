...
def switchFreq2(options):
    ...
    for freq in channels:
        record = Popen(['sudo', 'python3', GLOBAL_FILE_PATH+'/auto_record.py','-i {0}'.format(options['record_dir']), '-n {0}'.format(options['device']), '-q {0}'.format(options['drone_c_freq']), '-t {0}'.format(options['status']), '-v {0}'.format(options['env']), '-g {0}'.format(options['sdr_gain']),'-p {0}'.format(options['splitter']),'-x {0}'.format(options['duration_recording']),'-d {0}'.format(options['distance']),'-a {0}'.format(options['altitude']),'-c {0}'.format(freq),'-b {0}'.format(options['bw']),'-r {0}'.format(options['snr']), '-s {0}'.format(options['sampling_rate'])])
...
if __name__ == '__main__':
	...
	while True:
	    ...
	    print("Please input the center frequency of the target signal in MHz: ")
	    center_freq = input()
